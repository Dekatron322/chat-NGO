...
const BeneficiaryTransactionInfo = () => {
  ...
  useEffect(() => {
    ...
    if (beneficiaryId) {
      ...
      fetch(`https://api.donorsrec.chats.cash/payment/filter/payments-by-beneficiary/${beneficiaryId}/filter/`)
        ...
        .then((data: any) => {
          // Format the fetched data into the Order type
          const formattedOrders: Order[] = data.map((payment: Payment) => {
            const beneficiary = payment.beneficiarys && payment.beneficiarys.length > 0 ? payment.beneficiarys[0] : null
            const fullName =
...
export default BeneficiaryTransactionInfo
